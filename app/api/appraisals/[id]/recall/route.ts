...
export async function POST(
  ...
  try {
    ...
    if (!isAppraisalStatus(status) || (status !== "MANAGER_REVIEW" && status !== "SUBMITTED")) {
      return NextResponse.json(
        { error: "Appraisal must be in MANAGER_REVIEW to recall" },
        { status: 400 }
      );
    }

    if (appraisal.employee_id !== user.employee_id) {
      return NextResponse.json({ error: "Only the employee can recall submission" }, { status: 403 });
    }
